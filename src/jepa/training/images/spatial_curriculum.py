...
def logdet_richness_from_patches(
    ...
    *,
    ...
    sign, logabsdet = torch.linalg.slogdet(covariance + delta * eye)
    if torch.any(sign <= 0):
...
def score_frames_by_ras(
    ...
    *,
    ...
    try:
        ...
        richness = logdet_richness_from_patches(core, ref_patches, delta=richness_delta)
    ...
    metadata = {
        "ras/richness_logdet": float(richness.detach().cpu().item()),
