...
Added alongside (not instead of) :mod:`jepa.temporal_image_data`'s procedural
renderer. The hidden generative process is the same shape (a slow entity —
here, ``shape`` — and a faster context), but instead of rasterizing a frame
from continuous parameters, every observation is an *exact* image from
...
Reuses ``jepa.temporal_image_data.block_mask`` / ``build_static_spatial_dataset``
unmodified (both are generic over any ``[C, H, W]`` image tensor), and is
built to satisfy the same duck-typed dataset interface
(``.observations``, ``.entities``, ``.contexts``, ``config.trajectory_length``,
``config.observation_dim``) as the vector and procedural-image worlds, so
``jepa.temporal_data.make_temporal_pairs`` / ``make_hierarchical_pairs`` and
``jepa.temporal_training``'s per-epoch training helpers run unmodified here too.
"""
...
from jepa.temporal_image_data import (  # noqa: F401 -- re-exported for convenience
    StaticSpatialDataset,
    block_mask,
    build_static_spatial_dataset,
)
from jepa.temporal_shapes3d_config import (
