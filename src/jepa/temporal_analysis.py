...
from jepa.metrics import MetricValue, RidgeProbe, fit_ridge_probe
...
def fit_whitening(reference_latents: torch.Tensor, *, epsilon: float = 1e-6) -> WhiteningTransform:
    ...
    inv_sqrt = eigenvectors @ np.diag(1.0 / np.sqrt(np.clip(eigenvalues, epsilon, None))) @ eigenvectors.T
...
def compute_latent_spectrum(latents: torch.Tensor) -> LatentSpectrum:
    ...
    effective_rank = float(np.exp(-(positive * np.log(positive + 1e-12)).sum())) if trace > 0 else 0.0
