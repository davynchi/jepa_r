...
This is a parallel configuration tree to :mod:`jepa.configs.base`. It reuses the same
...
from jepa.configs.base import EMAConfig, OutputConfig, _merge_strict, derive_seed
...
class TemporalModelConfig:
    ...
    # 8 matches the vector world's true signal dimensionality ((num_entities-1)+
    # context_dim = 2+6) -- a real, if boundary-case, bottleneck rather than the
    # originally-specified 32 (== observation_dim), which we found empirically
    # left a random/untrained encoder indistinguishable from a trained one. See
    # validate_temporal_config's latent_dim warning.
    latent_dim: int = 8
    hidden_dim: int = 64
    hidden_layers: int = 1
    entity_latent_dim: int = 2
    context_latent_dim: int = 6
...
def validate_temporal_config(config: TemporalExperimentConfig) -> None:
    ...
    if model.architecture not in {"linear", "nonlinear", "cnn"}:
    ...
    # Soft warning, not a hard error: `_investigate_*.py` scripts deliberately
    # construct no-bottleneck configs to demonstrate this exact failure mode, so
    # rejecting it outright would break a legitimate diagnostic use case. But a
    # config built for a "real" run should almost always avoid this: once
    # latent_dim reaches the world's true signal dimensionality
    # ((num_entities - 1) + context_dim, i.e. the rank of the one-hot-plus-context
    # embedding), a generic linear/random encoder can preserve the full (E, C)
    # signal with no training at all, so post-hoc entity-subspace analysis can
    # find "structure" in an untrained encoder too -- see the bottleneck-confound
    # writeup that motivated this check.
    true_signal_dim = (data.num_entities - 1) + data.context_dim
    if model.kind in ("standard", "hierarchical") and model.latent_dim >= true_signal_dim:
        warnings.warn(
            f"model.latent_dim ({model.latent_dim}) >= the world's true signal "
            f"dimensionality ((num_entities - 1) + context_dim = {true_signal_dim}). "
            "A random/untrained encoder may be indistinguishable from a trained one "
            "in post-hoc entity-subspace analysis at this latent_dim; consider a "
            "smaller model.latent_dim, or run the random_encoder baseline alongside "
