...
(:mod:`jepa.training`), atomic artifact writers, representation metrics, and
...
module; it never touches :mod:`jepa.training`.
"""
...
from jepa.temporal_models import HierarchicalEncoderPredictor
...
from jepa.training import _atomic_json as _atomic_json  # noqa: F401 (re-exported helper)
...
def hierarchical_loss(
    ...
    entity_scale = _cov_trace(outputs.entity_latent) + hierarchical.epsilon
    context_scale = _cov_trace(outputs.context_latent) + hierarchical.epsilon
...
def _standard_epoch_loss(
    ...
    *,
    ...
    for indices in _batch_slices(
        ...
        context = pairs.context_view[indices].to(device)
        target = pairs.target_view[indices].to(device)
        if train:
