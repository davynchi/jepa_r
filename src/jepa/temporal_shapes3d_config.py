...
Added alongside, not instead of, the procedural renderer in
:mod:`jepa.temporal_image_config` / :mod:`jepa.temporal_image_data` -- both
remain available. This variant replaces the procedural rasterizer with an
exact lookup into DeepMind's 3D Shapes dataset
...
indices** rather than the continuous AR(1) process used by the procedural
renderer -- this needs no snapping/interpolation and indexes directly into the
dataset with zero approximation error.
"""
...
class Shapes3DDataConfig:
    ...
    def observation_dim(self) -> int:
        """Flattened 64x64x3 pixel count -- matches the vector/procedural-image
        worlds' duck interface so the same pair builders and training helpers
        run unmodified (see jepa.temporal_data / jepa.temporal_training)."""
        return 3 * 64 * 64
