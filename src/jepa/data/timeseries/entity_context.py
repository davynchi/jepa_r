...
from jepa.configs.timeseries import EntityContextDataConfig, TargetPairing
...
def make_temporal_pairs(
    ...
    *,
    ...
    is an **oracle** control (reported as ``oracle_same_entity`` in analysis
    scripts/plots) -- it uses the true, evaluation-only entity label to pick
    the replacement trajectory, so it is not something JEPA could reproduce
    unsupervised. It also draws the target from a different trajectory, but
    constrains that trajectory to have the *same* entity at the target time
    ...
    *preserving* entity persistence, isolating whether an effect attributed to
    ...
    about training on temporally contiguous data in general. Because it is an
    oracle, a strong result here is evidence about what's *achievable* when
    entity persistence is cleanly isolated, not evidence that unsupervised
    JEPA training discovers it unprompted -- that claim rests on the
    ``"temporal"`` vs ``"shuffled"`` comparison instead.
    """
