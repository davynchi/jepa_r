...
Mirrors :mod:`jepa.temporal_image_training` exactly: reuses
``_standard_epoch_loss``, ``_hierarchical_epoch_loss``, ``build_jepa_core``,
``build_hierarchical_core``, ``encode_split_standard`` from
:mod:`jepa.temporal_training` unmodified, because ``Shapes3DExperimentConfig``
...
/ ``config.observation_dim``) as the vector and procedural-image worlds.
"""
...
from jepa.temporal_shapes3d_data import build_shapes3d_dataset_splits
...
def train_shapes3d_experiment(
    config: Shapes3DExperimentConfig, *, seed_label: int | None = None
) -> Shapes3DTrainResult:
    started_at = _utc_now()
