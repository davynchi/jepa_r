...
from jepa.temporal_config import TemporalExperimentConfig, load_temporal_config  # noqa: E402
...
OUTPUT_ROOT = "/tmp/bottleneck_grid"
SEEDS = (0, 1, 2, 3, 4)
D_Z_VALUES = (4, 8, 16)
...
def base_config() -> TemporalExperimentConfig:
    return load_temporal_config(
        "configs/temporal_hierarchy_full.yaml",
...
def _full_analysis(
    ...
    *,
    ...
    cf = build_counterfactual_pairs(
        config.data, system, num_pairs=config.evaluation.counterfactual_pairs, seed=config.data.test_sample_seed + 999
    )
    with torch.no_grad():
    ...
    raw_inv = counterfactual_invariance(
        project(same_1, projection), project(same_2, projection), project(diff_1, projection), project(diff_2, projection)
    )
    ...
    eigen_w = solve_generalized_eigenproblem(scatter_w, epsilon=config.evaluation.covariance_epsilon)
    ...
    def _probe(features_train, features_test):
        clf = fit_entity_classifier(features_train, train_entity, config.data.num_entities, ridge=config.evaluation.probe_ridge)
        ctx_probe = fit_context_regressor(features_train, train_context, ridge=config.evaluation.probe_ridge)
    ...
    predictability = empirical_entity_predictability(train_ds.entities, horizon=config.training.prediction_horizon)
...
def run_trained(seed: int, d_z: int, p_e: float, horizon: int, pairing: str) -> dict:
    ...
    config = replace(
        ...
        training=replace(config.training, seed=seed, target_pairing=pairing, prediction_horizon=horizon),
    )
    result = train_temporal_experiment(config, seed_label=derive_seed(seed, "dz", d_z, "pe", p_e, "h", horizon, "pairing", pairing))
    checkpoint = torch.load(result.run_dir / "checkpoint.pt", map_location="cpu", weights_only=False)
...
def run_random_encoder(seed: int, d_z: int) -> dict:
    config = base_config()
    config = replace(config, model=replace(config.model, kind="random", latent_dim=d_z), training=replace(config.training, seed=seed))
    ...
    from jepa.temporal_data import generate_observation_system
    ...
    metrics = _full_analysis(
        ...
        config,
        system,
        extra={"condition": "random_encoder", "seed": seed, "d_z": d_z, "p_entity_switch": None, "horizon": None, "test_loss": None},
    )
    return metrics
...
def main() -> None:
    ...
    print(f"\nwrote {len(records)} records to {OUTPUT_ROOT}/records.json in {time.time()-start:.0f}s")
