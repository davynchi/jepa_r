#!/usr/bin/env python3
"""Ad-hoc diagnostic investigation for the procedural-renderer image world.

Mirrors ``_investigate_pairing_controls.py`` (the vector-world diagnostic)
exactly: same bottleneck question (is a random, untrained encoder
distinguishable from a trained one once you control for raw scale via
whitening?), same controls ({temporal, shuffled, oracle_same_entity} plus
random_encoder / random_orthogonal baselines), same metrics (D_same/D_diff
raw + whitened, full spectrum, entity/context probes on full latent and
complement). Runs at the quick preset scale (cheap) across a small
latent_dim grid, since the open question specific to images is whether the
same "random encoder looks as good as trained" failure mode shows up when
the observation map (rendering) is nonlinear rather than linear.
...
from jepa.temporal_image_config import ImageExperimentConfig, load_image_config  # noqa: E402
from jepa.temporal_image_data import (  # noqa: E402
    EntityContextImageTrajectoryDataset,
    build_image_counterfactual_pairs,
)
from jepa.temporal_image_training import train_temporal_image_experiment  # noqa: E402
from jepa.training import build_jepa_core  # noqa: E402

OUTPUT_ROOT = "/tmp/image_bottleneck_grid"
SEEDS = (0, 1, 2)
D_Z_VALUES = (8, 32)
...
def base_config() -> ImageExperimentConfig:
    return load_image_config(
        "configs/temporal_hierarchy_image_quick.yaml",
        overrides={"output.root": OUTPUT_ROOT, "output.overwrite": "on"},
    )


def _full_analysis(encoder, config: ImageExperimentConfig, *, extra: dict) -> dict:
    train_ds = EntityContextImageTrajectoryDataset(config.data, "train")
    test_ds = EntityContextImageTrajectoryDataset(config.data, "test")
    ...
    cf = build_image_counterfactual_pairs(
        config.data,
...
def run_trained(seed: int, d_z: int, pairing: str) -> dict:
    config = base_config()
    config = replace(
        config,
    ...
    result = train_temporal_image_experiment(
...
def main() -> None:
    ...
    for d_z in D_Z_VALUES:
        for pairing in PAIRINGS:
            for seed in SEEDS:
                metrics = run_trained(seed, d_z, pairing)
